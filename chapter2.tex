...
\textit{Breast cancer classification} divides breast cancer into categories according to different schemes\footnote{\url{http://www.breastpathology.info/}}, each serving a different purpose.
...
\clearpage
\noindent \Gls{NGS} grades breast carcinomas by adding up scores for:
...
\noindent each of which is given 1 to 3 points. The scores for each of these three 
criteria is then added together to give an overall final score and corresponding grade as follows \cite{damjanov2007cancer}:
...
\noindent Lower grade tumors, with a more favorable prognosis, can be treated less aggressively, and have a better survival rate.\newline

\textit{Mitosis} is a form of nuclear division of the mother cell into two daughter cells, genetically identical to each other and to their parent cell.
By this process, a cell, which has previously replicated each of its chromosomes, separates the chromosomes 
into two identical sets of chromosomes, each set in its own new nucleus  (see \cite{molBiologyCell, biology} for some details). 

Mitotic activity is one of the strongest prognosticators for invasive breast carcinoma. It is expressed as the number of mitotic figures per tissue area.
...
scanners (see Figure \ref{ch2:fig1}), in combination with digital slide viewers, aim to provide the experience of
...
\begin{figure}[!htbf]
 \centering
  \includegraphics[width=0.76\textwidth]{./images/aperio.png}
...
As previously stated, mitotic activity is one of the strongest prognosticators for invasive breast carcinoma and it
...
looking at specific examples of mitotic figures.
...
\begin{figure}[!hpbt]
 \centering
  \subfigure[source image]{
    \includegraphics[width=0.98\textwidth]{./images/A03_02th_new.jpg}
    \label{ch2:fig3:a}
   }\\
  \subfigure[mitoses]{
    \includegraphics[width=0.98\textwidth]{./images/A03_02_new.jpg}
    \label{ch2:fig3:b}
  }
...
\begin{figure}[!hpbt]
 \centering
  \subfigure[source image (detail)]{
    \includegraphics[width=0.98\textwidth]{./images/A03_02th_z_new.jpg}
    \label{ch2:fig3b:a}
   }\\
  \subfigure[image and mitoses (detail)]{
    \includegraphics[width=0.98\textwidth]{./images/A03_02_z_new.jpg}
    \label{ch2:fig3b:b}
  }
...
In this chapter we give an overview of the main aspects concerning \textit{image analysis} and in the following one (Section \ref{ch2:ML}) we analyze the \textit{machine learning} elements.
...
Digitized tissue histopathology has now become amenable to the application of computerized image analysis and machine learning techniques.
...
An interesting article \cite{BioImagingSW05} reviews each computational step that biologists encounter when dealing with digital images and the overall status of available software for bioimage informatics.
...
When dealing with digital image analysis and automatic identification of properties of an image, the idea of \textit{feature} and the 
functionality of a \textit{detector} are the main topics to be considered.\newline
...
An important set of features that can be computed on an image involve the concept of \textit{texture}.\newline
An image texture is a set of metrics designed to quantify the perceived structure of an image.
...
Here we briefly describe the main statistical feature algorithms.
...
\begin{equation}
\label{ch2:eq_lbp1}
 \textrm{LBP}_{P,R} = \sum_{p=0}^{P-1} s(g_p - g_c) \cdot 2^{p} \quad \textrm{where} \ s(x) = 
    \left\{
...
\begin{equation}
\label{ch2:eq_lbp2}
 \textrm{VAR}_{(P,R)} = \frac{1}{P} \sum_{p=0}^{P-1}(g_p - \mu)^{2} \quad \textrm{where} \ \mu = \sum_{p=0}^{P-1}g_p^2
\end{equation}
...
of image pixels within rectangular areas, using the so-called Haar-like features, a name that resembles the Haar wavelet adopted in other works \cite{objDetect03framework}.
...
We focus, in our analysis, on \textit{Pattern Recognition} and in particular on \textit{Supervised Learning} methods.
...
Binary classification is the task of classifying the members of a given set of objects into two groups on the basis of whether they have some properties or not \cite{scholkopf2002learning}.
...
\textit{Weka} \cite{dataMining_Weka, dataMining_Weka_upd} is a \textbf{FLOSS} general purpose data mining software tool developed by the 
Waikato University\footnote{\url{http://www.cs.waikato.ac.nz/ml/weka/}} which allows to implement a great variety of classifiers \cite{dataMiningBook}. It also has an interface
